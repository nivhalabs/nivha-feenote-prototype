...
/* Archive a policy fulfilment file. Used as the fallback when an Airtable
   attachment upload fails, so a paid order is never left with no copy of the
   documents. Files land under /POL/POL-XXXX/. */
async function uploadPolicyFile({ orderRef, filename, buffer }) {
  if (!DROPBOX_ENABLED) {
    console.log(`[dropbox dry-run] would upload ${orderRef}/${filename}`);
    return { ok: true, dryRun: true, path: `[dry-run] ${orderRef}/${filename}` };
  }
  const safeRef = String(orderRef || 'POL-UNKNOWN').replace(/[^A-Za-z0-9-]/g, '');
  const safeName = String(filename).replace(/[^A-Za-z0-9._-]/g, '-');
  const path = `${FOLDER}/POL/${safeRef}/${safeName}`;
  const token = await accessToken();
  const res = await fetch('https://content.dropboxapi.com/2/files/upload', {
    method: 'POST',
    headers: {
...
module.exports = { uploadFeeNote, uploadPolicyFile, DROPBOX_ENABLED, health };
