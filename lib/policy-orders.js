...
/* '2027-09-02' → '2 September 2027' — for buyer-facing emails. */
const longDate = s => {
  const d = new Date(s);
...
function registerFieldsFor({ orderRef, answers, order, clientCode, invoice }) {
  ...
  const fields = {
    ...
    /* Card orders activate the review when payment clears; invoice orders
       are fulfilled on account terms, so the review is active from order. */
    'Review status': order.hasReview && invoice ? 'Active' : 'None',
...
async function markPolicyPaid({ baseUrl, session }) {
  ...
  /* The annual review renews by payment link, so nothing is set up with
     Stripe here — the first year was charged in this payment, and the office
     sends a renewal link from the reviews queue about 30 days before it
     falls due. */
...
/* A renewal payment link was paid. The review runs on a fixed twelve-month
   cycle, so the new due date is twelve months on from the existing one — not
   from the day the buyer happened to pay. */
async function markReviewRenewalPaid({ baseUrl, session }) {
  const meta = session.metadata || {};
  const recordId = meta.recordId;
  if (!recordId) throw new Error('Renewal session carries no record id');
  const rec = await register.getOrder(recordId);
  if (!rec) throw new Error('Register record not found for renewal');
  const paymentId = String(session.payment_intent || session.id || '');
  /* Webhooks retry — a payment already noted is not applied twice. */
  if (paymentId && String(rec.fields['Notes'] || '').includes(paymentId)) {
    return { alreadyRecorded: true, recordId, orderRef: rec.fields['Order ref'] || '' };
  }
  const oldDue = rec.fields['Review due'] || '';
  const base = oldDue && !isNaN(new Date(oldDue)) ? new Date(oldDue) : new Date();
  base.setMonth(base.getMonth() + 12);
  const newDue = dateOnly(base);
  await register.patchOrder(recordId, { 'Review due': newDue, 'Review status': 'Active' });
  await addNote(recordId, `Annual review renewed ${dateOnly(new Date())} \u2014 paid by payment link (${paymentId}), review due extended to ${newDue}`);
  if (session.payment_link) {
    try { await stripe.deactivatePaymentLink(String(session.payment_link)); }
    catch (e) { console.error('payment link deactivation failed (non-fatal):', e.message); }
  }
  const orderRef = rec.fields['Order ref'] || meta.orderRef || '';
  if (rec.fields['Email']) {
    try {
      await policyReviewRenewedEmail({
        baseUrl, to: rec.fields['Email'], orderRef,
        organisation: rec.fields['Organisation'] || '',
        newDueDate: longDate(newDue), bcc: OFFICE_EMAIL
      });
    } catch (e) { console.error('renewal receipt email failed (non-fatal):', e.message); }
  }
  return { alreadyRecorded: false, recordId, orderRef, newDue };
}
...
function mount(app, { baseUrl }) {
  ...
  /* The reviews queue — every order with the review service running, soonest
     due first, so the office can send renewal links about 30 days ahead. */
  app.get('/api/admin/policy/reviews', async (req, res) => {
    if (!guard(req, res)) return;
    try {
      const records = await register.listActiveReviews();
      const today = new Date();
      const reviews = records.map(r => {
        const due = r.fields['Review due'] || '';
        const daysToDue = due ? Math.round((new Date(due) - today) / 86400000) : null;
        ...
        return {
          ...
          reviewDue: due,
          daysToDue,
          vatTreatment: r.fields['VAT treatment'] || '',
          linkSent: /Renewal link sent/.test(notes)
        };
      });
      res.json({ ok: true, reviews, dryRun: register.DRY_RUN });
    } catch (err) {
      console.error('GET /api/admin/policy/reviews failed:', err.message);
      res.status(502).json({ ok: false, error: 'Could not load the reviews.' });
    }
  });

  /* Create the renewal payment link and email it to the buyer. */
  app.post('/api/admin/policy/reviews/:id/renewal-link', async (req, res) => {
    if (!guard(req, res)) return;
    try {
      if (!isRecordId(req.params.id)) return res.status(400).json({ ok: false, error: 'Bad record id' });
      if (stripe.SIMULATED) return res.status(400).json({ ok: false, error: 'Payments are simulated in this environment.' });
      const rec = await register.getOrder(req.params.id);
      if (!rec) return res.status(404).json({ ok: false, error: 'Order not found.' });
      if ((rec.fields['Review status'] || '') !== 'Active') return res.status(400).json({ ok: false, error: 'The review service is not active on this order.' });
      const roi = (rec.fields['VAT treatment'] || '') === 'ROI reverse charge';
      const orderRef = rec.fields['Order ref'] || '';
      const link = await stripe.createReviewRenewalLink({ roi, orderRef, recordId: rec.id });
      const due = rec.fields['Review due'] || '';
      /* The details we hold, so the buyer can confirm or correct them. */
      let policyOwner = '', dpContact = '';
      try {
        const saved = JSON.parse(rec.fields['Answers JSON'] || '{}');
        policyOwner = clean((saved.answers && saved.answers.details || {}).policyOwner, 120);
        dpContact = clean((saved.answers && saved.answers.details || {}).dpContact, 120);
      } catch (e) { /* older records without parseable answers still get the email */ }
      const amountLine = roi
        ? '\u00a360.00, with VAT accounted for by your organisation under the reverse charge'
        : '\u00a372.00 including VAT (\u00a360.00 + VAT)';
      let emailed = false;
      if (rec.fields['Email']) {
        try {
          await policyReviewRenewalEmail({
            baseUrl, to: rec.fields['Email'], orderRef,
            organisation: rec.fields['Organisation'] || '',
            dueDate: longDate(due), amountLine, payUrl: link.url,
            policyOwner, dpContact, bcc: OFFICE_EMAIL
          });
          emailed = true;
        } catch (e) { console.error('renewal email failed (non-fatal):', e.message); }
      }
      await addNote(rec.id, `Renewal link sent ${dateOnly(new Date())} for review due ${due}: ${link.url} (${link.id})${emailed ? '' : ' \u2014 email failed, send manually'}`);
      res.json({ ok: true, url: link.url, emailed });
    } catch (err) {
      console.error('renewal link failed:', err.message);
      res.status(502).json({ ok: false, error: 'Could not create the renewal link.' });
    }
  });
...
<h1 style="margin-top:34px">Annual reviews</h1>
<p class="lede">Every order with the review service running, soonest due first. About 30 days before a review falls due, send the renewal link \u2014 the buyer gets an email with a Stripe payment link, and paying extends the review by twelve months from the due date. Nothing is charged automatically.</p>
<div id="reviews"><p class="empty">Loading reviews\u2026</p></div>
<script>
 ...
 function loadReviews() {
   api('/api/admin/policy/reviews').then(function (d) {
     var el = document.getElementById('reviews');
     if (!d.ok) { el.innerHTML = '<p class="empty">Could not load the reviews.</p>'; return; }
     if (!d.reviews.length) { el.innerHTML = '<p class="empty">No orders have the review service running.</p>'; return; }
     el.innerHTML = d.reviews.map(function (r) {
       var days = (r.daysToDue == null) ? '' :
         (r.daysToDue < 0 ? Math.abs(r.daysToDue) + ' days overdue' : 'in ' + r.daysToDue + ' days');
       var dueSoon = r.daysToDue != null && r.daysToDue <= 30;
       return '<div class="order" id="r-' + r.id + '">' +
         '<div class="ref">' + esc(r.orderRef) + ' \\u00b7 ' + esc(r.organisation) + '</div>' +
         '<p class="meta">' + esc(r.contact) + ' &lt;' + esc(r.email) + '&gt; \\u00b7 ' + esc(r.vatTreatment) + '<br>' +
         'Review due ' + esc(r.reviewDue || '\\u2014') + (days ? ' (' + days + ')' : '') +
         (r.linkSent ? ' \\u00b7 renewal link already sent' : '') + '</p>' +
         (dueSoon && !r.linkSent ? '<div class="blocked">Due within 30 days \\u2014 time to send the renewal link.</div>' : '') +
         '<button data-renew="' + r.id + '">' + (r.linkSent ? 'Send a fresh renewal link' : 'Email the renewal link') + '</button>' +
         '<p class="msg" id="rm-' + r.id + '"></p></div>';
     }).join('');
     el.querySelectorAll('[data-renew]').forEach(function (b) {
       b.addEventListener('click', function () {
         var id = b.getAttribute('data-renew');
         if (!confirm('Create a Stripe payment link and email it to the buyer?')) return;
         b.disabled = true;
         api('/api/admin/policy/reviews/' + id + '/renewal-link', { method: 'POST', body: '{}' })
           .then(function (r) {
             var m = document.getElementById('rm-' + id);
             if (r.ok) { m.innerHTML = (r.emailed ? 'Emailed to the buyer. ' : 'Email failed \\u2014 send it manually. ') + 'Link: <a href="' + esc(r.url) + '" target="_blank" rel="noopener">' + esc(r.url) + '</a>'; }
             else { m.textContent = r.error || 'Could not create the link.'; b.disabled = false; }
           });
       });
     });
   });
...
module.exports = {
  mount, markPolicyPaid, markReviewRenewalPaid, runFulfilment, adminOk,
  ADMIN_TOKEN, OFFICE_EMAIL, cardPaymentsEnabled
};
