...
function buildChildren(v) {
  ...
  c.push(p(`1.3 This policy does not form part of any contract of employment and may be amended following consultation. The current version is available from the ${roleRef(owner)}.`));
  ...
  c.push(p(`12.4 Records created under this policy are kept only as long as necessary for the purposes described here and are then securely destroyed. Individuals have the rights given by data protection law, including access to their own records. Questions should be directed to the ${roleRef(v.dpContact)}.`));
  ...
  c.push(p('14.3 Nothing in this policy prevents the organisation taking precautionary action, including suspension on full pay, while a matter is investigated. Precautionary suspension is a neutral act, not a disciplinary sanction.'));
  if (testing) c.push(p('14.4 Where employment continues following a confirmed positive result, the organisation may require unannounced follow-up testing at intervals for a defined period, normally up to six months, as a condition of remaining in or returning to the role.'));
