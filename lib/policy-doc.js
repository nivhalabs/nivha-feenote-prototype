...
function buildChildren(v) {
  ...
  if (v.sector === 'transport') {
    const regimes = [];
    if (hasNI || hasGB) regimes.push('in the United Kingdom, the Transport and Works Act 1992 (railways and other guided transport systems, in Great Britain) and the Railways and Transport Safety Act 2003 (aviation and shipping)');
    if (isROI) regimes.push('in the Republic of Ireland, the Railway Safety Act 2005 (railway safety-critical workers)');
    c.push(p(`4.${clause++} Some transport activities are covered by their own statutory alcohol and drugs regimes with prescribed limits and testing powers \u2014 ${regimes.join('; and ')}. Where such a regime applies to the organisation\u2019s operations, it operates alongside this policy and takes precedence over it.`));
  }
  ...
  if (!testing) {
    ...
    if (v.testingTypes.includes('post_incident')) c.push(p('**Post-incident.** Testing may be required following an accident, near miss or significant operational incident, where substance involvement cannot reasonably be ruled out. Where time has passed between the incident and the test, the organisation may ask the testing provider for an expert assessment of the likely level at the time of the incident, based on the later result (a back-calculation).'));
    ...
    if (v.sampleTypes.includes('breath')) c.push(p('Where breath testing for alcohol is used, any reading above the applied standard is followed by a second confirmatory reading after a short interval, and the lower of the two readings is taken as the result.'));
  ...
  if (v.support.selfReferral) {
    c.push(p(`11.${sup++} Anyone who believes they have a problem with alcohol or drugs is strongly encouraged to come forward. A voluntary disclosure made before selection for testing, and before any incident or performance concern arises, will be treated as a health matter and not as a breach of this policy. The organisation will agree a support plan, which may include adjusted duties, time off for treatment and return-to-work testing.`));
  }
  ...
  c.push(p('12.2 Drug and alcohol test results and related health information are special category data. ' + dpBasis + ' Testing is carried out only where it is a necessary and proportionate way of meeting these duties, and only the minimum information needed is collected.'));
  ...
  if (testing) c.push(p('14.5 Where employment continues following a confirmed positive result, the organisation may require unannounced follow-up testing at intervals for a defined period, normally up to six months, as a condition of remaining in or returning to the role.'));
