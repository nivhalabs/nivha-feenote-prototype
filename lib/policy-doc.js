...
function buildChildren(v) {
  ...
  if (!testing) {
    ...
    c.push(p(`Screening results that indicate the possible presence of a substance ("non-negative" results) are not treated as positive. Every non-negative screening result${v.sampleTypes.includes('breath') ? ', other than a breath alcohol reading (which is confirmed by the second reading described above),' : ''} is sent for laboratory confirmation at a laboratory accredited to ISO/IEC 17025 for the relevant methods, with results assessed against the confirmation cut-off levels in ${schedRef}.`));
