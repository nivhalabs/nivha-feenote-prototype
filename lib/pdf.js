...
function courtRatesBlock(doc) {
  /* Lives on page 2+ with the test descriptions — kept off page 1 so the
     headline fee note is not dominated by court attendance rates. */
  const need = 15 + COURT_RATES.lines.length * 12.5 + 34;
  if (doc.y + need > PAGE.h - PAGE.margin - 26) { doc.addPage(); brandBand(doc); doc.y = PAGE.margin; }
  doc.y += 8;
...
/* ---------- entry point ---------- */
/* NIVHA convention: PDF password is the reference with an underscore,
   e.g. fee note CCN-1234 opens with CCN_1234. Set PDF_PASSWORDS=off to
   disable (e.g. for local debugging). */
function pdfPassword(ref) {
  if (process.env.PDF_PASSWORDS === 'off') return null;
  return String(ref).replace('-', '_');
}

function generateFeeNote(data, outPath) {
  return new Promise((resolve, reject) => {
    const pw = pdfPassword(data.ref);
    const doc = new PDFDocument({ size: 'A4', margin: PAGE.margin, bufferPages: true,
      ...(pw ? { pdfVersion: '1.7ext3', userPassword: pw, ownerPassword: process.env.PDF_OWNER_PASSWORD || pw } : {}),
      info: {
    ...
    pageOneFooter(doc);
    const hasPanelPages = panelPages(doc, data);
    if (data.route !== 'private') {
      if (!hasPanelPages) { doc.addPage(); brandBand(doc); doc.y = PAGE.margin; }
      courtRatesBlock(doc);
    }
...
module.exports = { generateFeeNote, pdfPassword, CATALOGUE, COURT_RATES };
