...
/* Intake form field that carries the fee note summary. Unlike admin notes,
 * intake form answers sync out to third-party calendars (Outlook). */
const FIELD_FEENOTE = process.env.ACUITY_FIELD_FEENOTE || '';
...
/* Intake forms and their field IDs — used once during setup. */
const getForms = () => api('GET', '/forms');
...
const createAppointment = ({ datetime, firstName, lastName, email, phone, notes, typeId = TYPE_BELFAST }) =>
  api('POST', '/appointments', {
    ...
    phone: phone || '',
    notes: notes || '',
    ...(FIELD_FEENOTE ? { fields: [{ id: Number(FIELD_FEENOTE), value: notes || '' }] } : {})
  });

module.exports = { SIMULATED, TYPE_BELFAST, FIELD_FEENOTE, getTypes, getCalendars, getForms, getDates, getTimes, createAppointment };
