...
async function send({ to, subject, textBody, htmlBody, tag, attachments }) {
  if (EMAIL_DRY_RUN) {
    const att = attachments ? ` attachments=[${attachments.map(a => a.Name).join(', ')}]` : '';
    console.log(`[email dry-run] to=${to} tag=${tag}${att} subject="${subject}"\n${textBody}`);
...
function feeNoteEmail({ baseUrl, to, reference, isPrivate, paid, pdfBuffer, downloadLink }) {
  ...
  return send({ to, subject, textBody, htmlBody, tag: 'fee-note', attachments });
}
