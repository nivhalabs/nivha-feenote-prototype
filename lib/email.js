...
function policyOrderEmail({ baseUrl, to, orderRef, organisation, order, invoice, bcc }) {
  ...
  const reviewLine = order.hasReview
    ? 'Your annual review starts twelve months from today. We will be in touch about 30 days before your review date.'
    : '';
  const textBody = [
    'Hello,',
    '',
...
function policyDeliveryEmail({ baseUrl, to, orderRef, organisation, files, hasReview, portalUrl, bcc }) {
  ...
  const reviewLine = hasReview
    ? ['Your annual review is active. We will be in touch about 30 days before your review date.',
      portalUrl ? `You can cancel the review at any time through your billing page: ${portalUrl}` : 'To cancel the review, reply to this email before your renewal date.'].join(' ')
    : '';
