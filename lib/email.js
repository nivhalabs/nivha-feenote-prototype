...
function policyReviewRenewalEmail({ baseUrl, to, orderRef, organisation, dueDate, amountLine, payUrl, bcc }) {
...
function policyReviewRenewedEmail({ baseUrl, to, orderRef, organisation, newDueDate, bcc }) {
  ...
  const textBody = ['Hello,', '', bodyText, '', FOOTER_TEXT].join('\n');
  const htmlBody = layout(baseUrl, `
    <p style="margin:0 0 14px;">Hello,</p>
    <p style="margin:0;">${escHtml(bodyText)}</p>`);
